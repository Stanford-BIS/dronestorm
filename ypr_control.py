...
#       roll = roll + droll * dt  #add the imu derivative data to interpolate between frames
        ...
        d_error_prev[d_error_idx] = (error_roll - error_roll_prev) / dt
        d_error_idx += 1
        d_error_idx %= 3
        d_error_roll = np.mean(d_error_prev)
...
#       pitch = pitch + dpitch * dt  #add the imu derivative data to interpolate between frames
        ...
        d_error_prev[d_error_idx] = (error_pitch - error_pitch_prev) / dt
        d_error_idx += 1
        d_error_idx %= 3
        d_error_pitch = np.mean(d_error_prev)
